...
_HOUGHVOTING_NUM_INLIER = 100 # 500
_HOUGHVOTING_DIRECTION_INLIER = 0.9
_LABEL2MASK_THRESHOL = 100 # 500
...
def loss_cross_entropy(scores, labels):
    ...
    """
    class_cross_entropy = -1 * (labels * torch.log(scores + 1e-10))
    class_loss = torch.div(torch.sum(class_cross_entropy, dim=(0, 2, 3)), torch.sum(labels)+1e-10)

    cross_entropy = -torch.sum(labels * torch.log(scores + 1e-10), dim=1)
    loss = torch.div(torch.sum(cross_entropy), torch.sum(labels)+1e-10)
    return loss, class_loss[1:]
...
def loss_Rotation(pred_R, gt_R, label, model):
    ...
    class_loss = ((pred_points - gt_points) ** 2).sum(dim=2).sqrt().mean(dim=1)
    
    all_class_loss = torch.zeros(model.shape[0]).to(device = device, dtype = torch.float)
    all_class_loss[label - 1] = class_loss

    return class_loss.mean(), all_class_loss
...
def HoughVoting(label, centermap, num_classes=10):
    ...
    centers = torch.zeros(batches, num_classes, 2).to(device = label.device, dtype=torch.float32)
    depths = torch.zeros(batches, num_classes).to(device = label.device, dtype=torch.float32)
