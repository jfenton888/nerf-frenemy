"""
Task model for a FrenemyModel using PoseCNN with the PROPS-NeRF dataset.
"""
...
class PoseCNNTask(Task):
    ...
    def posecnn_dict(self, outputs, batch, image_idx):
        """
        Create the input dictionary for the PoseCNN model.
    ...
    def get_metrics_dict(self, metrics_dict, outputs, batch):
        ...
        There are not specific needed metrics for PoseCNN, so we just return the metrics_dict.
        """
    ...
    def get_image_metrics_and_images(
        ...
        posecnn_dict = self.posecnn_dict(outputs, batch, None) # Only one image in the batch

        with torch.no_grad():
            original_loss = self.posecnn_model({"rgb": rgb.permute(2, 0, 1).unsqueeze(0), **posecnn_dict})
            pred_loss = self.posecnn_model({"rgb": pred_rgb.permute(2, 0, 1).unsqueeze(0), **posecnn_dict})
        
        metrics_dict.update({f"original_{k}": v for k, v in original_loss.items()})
        metrics_dict.update({f"predicted_{k}": v for k, v in pred_loss.items()})

        gt_segmentation = posecnn_dict["label"][0]
        ...
        pose_dict = format_gt_RTs(batch["RTs"].cpu().numpy())
